...
class Moneytigo extends PaymentModule {
  ...
  public function __construct() {
    ...
    $this->BaseWebPaymentSubscription = 'https://checkout.moneytigo.com/pay/subscription/token/';
    ...
    $this->EmbeddedLibrary = 'https://checkout.moneytigo.com/dist/js/moneytigapp.js';
    $this->EmbeddedCss = 'https://checkout.moneytigo.com/dist/css/app.css';
  ...
  public function uninstall() {
    ...
    $moneytigo_install->deleteConfiguration( $this->name );
    //  $moneytigo_install->uninstallOrderStates( $this->name );
    if ( _PS_VERSION_ < '1.7' ) {
  ...
  public function hookPaymentOptions( $params ) {
    ...
    if ( Configuration::get( 'MONEYTIGO_INTEGRATED' ) == 'on' ) {
      ...
      if ( $optionPaymentD[ 'result' ] ) {
        ...
        echo '<div class="alert alert-danger"><b>MoneyTigo Standard: </b>' . $optionPaymentD[ 'message' ] . '</div>';
      }
    }
    /** Pnf payment   **/
    $PnfActive = $moneytigoCore::getleaseactive( $cart->getordertotal( true ) );
  ...
  public function getEmbeddedPaymentOption( $params, $error = NULL ) {
    $moneytigoCore = new moneytigoCore();
    $Tokenise = $moneytigoCore::moneytigo_constructPayment( $params );
    ...
    $embeddedOption->setModuleName( 'moneytigoEmbedded' )->setBinary( true )->setAdditionalInformation( $this->context->smarty->fetch( 'module:moneytigo/views/templates/front/17/embedded.tpl' ) )->setLogo( Media::getMediaPath( _PS_MODULE_DIR_ . $this->name . '/views/img/carte.png' ) );
  ...
  public function optionPaymentPNF( $params, $lease, $error = NULL, $optionD = NULL ) {
    $moneytigoCore = new moneytigoCore();
    $Tokenise = $moneytigoCore::moneytigo_constructPayment( $params, $lease );
    if ( $Tokenise[ 'success' ] == false ) {
      $error = $Tokenise[ 'error_message' ];
    }
  ...
  public function hookPaymentReturn( $params ) {
    ...
    if ( $order->current_state == Configuration::get( 'MONEYTIGO_OS_ACCEPTED' ) ) {
      $paymenttitle = $this->l( 'Credit card' );
    } elseif ( $order->current_state == Configuration::get( 'MONEYTIGO_OS_ACCEPTED_P2F' ) ) {
      $paymenttitle = $this->l( 'Credit card in 2 times' );
    }
    elseif ( $order->current_state == Configuration::get( 'MONEYTIGO_OS_ACCEPTED_P3F' ) ) {
      $paymenttitle = $this->l( 'Credit card in 3 times' );
    }
    elseif ( $order->current_state == Configuration::get( 'MONEYTIGO_OS_ACCEPTED_P4F' ) ) {
...
*/
  ...
  public function getContent() {
    ...
    if ( !empty( Tools::getValue( 'MONEYTIGO_ADMIN_ACTION' ) ) ) {
      ...
      Configuration::updateValue( 'MONEYTIGO_GATEWAY_P2F', Tools::getValue( 'MONEYTIGO_GATEWAY_P2F' ) ); //Enable or not split payment 3steps
      Configuration::updateValue( 'MONEYTIGO_GATEWAY_P3F', Tools::getValue( 'MONEYTIGO_GATEWAY_P3F' ) ); //Enable or not split payment 3steps
      Configuration::updateValue( 'MONEYTIGO_GATEWAY_P4F', Tools::getValue( 'MONEYTIGO_GATEWAY_P4F' ) ); //Enable or not split payment 3steps
      ...
      if ( Tools::getValue( 'MONEYTIGO_TRIGGER_P2F' ) && ( ( int )Tools::getValue( 'MONEYTIGO_TRIGGER_P2F' ) < 50 && Tools::getValue( 'MONEYTIGO_GATEWAY_P2F' ) == "on" ) ) {
        $nberror = $nberror + 1;
        ...
        Configuration::updateValue( 'MONEYTIGO_TRIGGER_P2F', Tools::getValue( 'MONEYTIGO_TRIGGER_P2F' ) );
      }
      ...
      if ( Tools::getValue( 'MONEYTIGO_TRIGGER_P4F' ) && ( ( int )Tools::getValue( 'MONEYTIGO_TRIGGER_P4F' ) < 50 && Tools::getValue( 'MONEYTIGO_GATEWAY_P4F' ) == "on" ) ) {
        $nberror = $nberror + 1;
        ...
        Configuration::updateValue( 'MONEYTIGO_TRIGGER_P4F', Tools::getValue( 'MONEYTIGO_TRIGGER_P4F' ) );
      }


      if ( !is_numeric( Tools::getValue( 'MONEYTIGO_FEE_P2F' ) ) && Tools::getValue( 'MONEYTIGO_FEE_P2F' ) ) {
        $nberror = $nberror + 1;
        $error .= "<b>P2F</b> - $this->l('Non-numeric value for the two times payment processing fee. (e.g. 1.00) for 1%.')<br>";
      } else {
        Configuration::updateValue( 'MONEYTIGO_FEE_P2F', Tools::getValue( 'MONEYTIGO_FEE_P2F' ) );
      }
      ...
      if ( !is_numeric( Tools::getValue( 'MONEYTIGO_FEE_P4F' ) ) && Tools::getValue( 'MONEYTIGO_FEE_P4F' ) ) {
        $nberror = $nberror + 1;
        $error .= "<b>P4F</b> - $this->l('Non-numeric value for the four times payment processing fee. (e.g. 1.00) for 1%.')<br>";
      } else {
        Configuration::updateValue( 'MONEYTIGO_FEE_P4F', Tools::getValue( 'MONEYTIGO_FEE_P4F' ) );
      }
    ...
    $seuil_p2f = Tools::safeOutput(
      Tools::getValue( 'MONEYTIGO_TRIGGER_P2F', Configuration::get( 'MONEYTIGO_TRIGGER_P2F' ) )
    );
    ...
    $seuil_p4f = Tools::safeOutput(
      Tools::getValue( 'MONEYTIGO_TRIGGER_P4F', Configuration::get( 'MONEYTIGO_TRIGGER_P4F' ) )
    );
    $fee_p2f = Tools::safeOutput(
      Tools::getValue( 'MONEYTIGO_FEE_P2F', Configuration::get( 'MONEYTIGO_FEE_P2F' ) )
    );
    $fee_p3f = Tools::safeOutput(
      Tools::getValue( 'MONEYTIGO_FEE_P3F', Configuration::get( 'MONEYTIGO_FEE_P3F' ) )
    );
    $fee_p4f = Tools::safeOutput(
      Tools::getValue( 'MONEYTIGO_FEE_P4F', Configuration::get( 'MONEYTIGO_FEE_P4F' ) )
    );
    ...
    if ( Tools::getValue( 'MONEYTIGO_GATEWAY_P2F', Configuration::get( 'MONEYTIGO_GATEWAY_P2F' ) ) == "on" ) {
    ...
    if ( Tools::getValue( 'MONEYTIGO_GATEWAY_P4F', Configuration::get( 'MONEYTIGO_GATEWAY_P4F' ) ) == "on" ) {
